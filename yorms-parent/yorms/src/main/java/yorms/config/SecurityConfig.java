...
public class SecurityConfig extends WebSecurityConfigurerAdapter {
	...
	protected void configure(HttpSecurity http) throws Exception {
		...
		http.sessionManagement().invalidSessionUrl("/login").maximumSessions(5).maxSessionsPreventsLogin(true)
				.expiredUrl("/login");
		http.logout().permitAll();
		http.authorizeRequests().antMatchers("/approve/**").hasAuthority("系统管理员").anyRequest().authenticated();
